...
public class ZeebeMongoClient {
    ...
    private List<Tuple<String, UpdateOneModel<Document>>> bulkOperations;
    private final MongoExporterConfiguration configuration;
    private final MongoClient client;
    public static final String COL_DELIMITER = "_";

    public ZeebeMongoClient(Logger log, MongoExporterConfiguration configuration) {
        this.log = log;
        this.configuration = configuration;
        this.client = createClient();
        this.bulkOperations = new ArrayList<>();

        log.info("ZeebeMongoClient created - {}", client);
    }

    private MongoClient createClient() {
        // Create credentials
        MongoCredential credential = MongoCredential.createCredential(
                configuration.username, "admin", configuration.password.toCharArray());

        // Configure client settings
        MongoClientSettings settings = MongoClientSettings.builder()
                .credential(credential)
                .applyToClusterSettings(builder ->
                        builder.hosts(Arrays.asList(new ServerAddress(configuration.host, configuration.port))))
                .build();
